...
class KD_IT(object):
  ...
  def renderer(self, x, final_res=128):
  ...
  def build(self, inputs, output_tensors=False, build_loss=True):
    ...
    crude_output, mask = self.renderer(joint_embedding)
  ...
  def evaluate(self, opts, test_dataset, handle_pl, checkpoint_fname, vars_to_restore=None, reset_global_step=True):
    tf.logging.set_verbosity(tf.logging.INFO)
    # define iterators
    test_iterator = test_dataset.make_initializable_iterator()
    global_init = tf.global_variables_initializer()
